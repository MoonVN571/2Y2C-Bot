...
module.exports = {
    ...
    async execute(client, message, args) {
        if (!args[0]) return message.reply({ content: "Cách dùng: " + client.config.client.config.prefix + "setup <livechat/connection/restart/role>", allowedMentions: { repliedUser: false } });
        ...
        switch (args[0]) {
            case "livechat": {
                if (!args[1]) return message.reply({ content: "Cách dùng: " + client.config.prefix + "setup livechat <#Kênh>", allowedMentions: { repliedUser: false } });
                ...
                if (!data.get('livechat')) return message.reply({ content: "Bạn đã setup kênh log livechat rồi. Xoá kênh loglivechat bằng lệnh " + client.config.prefix + "delete livechat <Kênh>", allowedMentions: { repliedUser: false } })
            ...
            case "connection": {
                if (!args[1]) return message.reply({ content: "Cách dùng: " + client.config.prefix + "setup connection <#Kênh>", allowedMentions: { repliedUser: false } });
                ...
                if (!data.get('connection')) return message.reply({ content: "Bạn đã kênh log connection rồi. Xoá kênh log connection bằng lệnh " + client.config.prefix + "delete livechat <#Kênh>", allowedMentions: { repliedUser: false } })
                ...
                if (data.get('connection'))
                    return message.reply({
                        content: "Bạn đã setup kênh connection rồi. Xoá kênh connection bằng lệnh " + client.config.prefix + "delete connection <Kênh>",
            ...
            case "restart-role": {
                if (!args[1]) return message.reply({ content: "Cách dùng: " + client.config.prefix + "setup restart-role <Tên role>", allowedMentions: { repliedUser: false } });
                ...
                if (data.get('restart-role'))
                    return message.reply({
                        content: "Bạn đã setup role restart rồi. Xoá role restart bằng lệnh " + client.config.prefix + "delete restart-role <Tên role>",
                        allowedMentions: { repliedUser: false }
                    });
            ...
            case "restart": {
                if (!args[0]) return message.reply({ content: "Cách dùng: " + client.config.prefix + "setup restart <#Kênh>", allowedMentions: { repliedUser: false } });
                ...
                if (!data.get('restart')) return message.reply({ content: "Bạn đã setup restart rồi. Xoá restart bằng lệnh " + client.config.prefix + "delete restart <Kênh>", allowedMentions: { repliedUser: false } })
            ...
            default: {
                message.reply({ content: "Không thấy setting này. Cú pháp: " + client.client.config.prefix + "setup <chat/connection/restart/restart-role>", allowedMentions: { repliedUser: false } });
            }
        }
    }
}
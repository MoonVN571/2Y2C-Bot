...
function inject (bot, options) {
  ...
  bot.chatPatterns = [
    {
      pattern: new RegExp(`^${USERNAME_REGEX} whisper(?: whisper to)?:? (.*)$`),
      type: 'whisper',
  ...
  bot._client.on('chat', (packet) => {
    function checkForChatPatterns (msg) {
      ...
      for (const { pattern, type } of bot.chatPatterns) {
        ...
        /*
        if(stringMsg.startsWith('[Server]')) break
        if(stringMsg.startsWith("CS:")) break // break cs message
        if(stringMsg.startsWith("[PistonChat]")) break
        if(stringMsg.startsWith("[LP]")) break // break LP error
        if(stringMsg.startsWith("Achievement:")) break
        if(stringMsg.startsWith("[Broadcast]")) break // ess broadcast
        if(stringMsg.startsWith("[UUID]")) break // uuid of check
        */
